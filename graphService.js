...
export async function fetchLivePilotData() {
  ...
  onboardingRows.slice(onbStartRow).forEach(row => {
    ...
    // instead of a real ID or leaving it blank. Normalize that to an empty
    // string for consistency, but — IMPORTANT, v2.1 — do NOT discard the row.
    // A row with both Owner and Authorize blank is no longer just noise: it's
    // the signal that merchant status was explicitly granted to whoever is in
    // the Destination column (see onboardingLogic.js's dash-row-grant rule).
    // Dropping these rows here would make that rule silently never fire.
    ...
    const resolvedAgentName = authorizeId ? (accountIdToName.get(authorizeId) || null) : null;

    let ownerId = row[c_owner] ? String(row[c_owner]).trim() : "";
    if (ownerId === "—" || ownerId === "-" || ownerId === "0") ownerId = "";
