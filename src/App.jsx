...
import { dedupeOnboardingEvents, getAllMerchantNames, getConsumersForMerchant, getAllConsumersWithMerchant, buildAgentLookup, isAgentName } from "./onboardingLogic";
import { calculateMerchantIncentive, calculateConsumerIncentive } from "./incentiveEngine";
import {
  loadIncentiveConfigs, saveIncentiveConfigs,
  loadConsumerIncentiveConfigs, saveConsumerIncentiveConfigs,
  loadDashboardTargets, saveDashboardTargets,
  loadPaidTriggers, savePaidTriggers,
  loadPaidBaselines, savePaidBaselines,
  loadAgents, saveAgents,
  DEFAULT_INCENTIVE_CONFIG, DEFAULT_CONSUMER_INCENTIVE_CONFIG,
} from "./phaseConfig";
...
const C = {
  bgDark: "#f7f5ef", panelDark: "#ffffff", panelDark2: "#f0ede4", forest: "#1f3d35",
  forestLight: "#2d5a4d", gold: "#c9941f", goldText: "#8a6206", goldLight: "#e8c468", cream: "#1a1a1a",
  textMuted: "#4a4a4a", textFaint: "#6b6b63", border: "#e2ddd0",
  red: "#c2492f", amber: "#946a07", green: "#367a57", blue: "#2f6fa3",
};
...
function KPICard({ label, value, sub, color = C.gold, icon }) {
  return (
    <div style={{ background: C.panelDark, border: `1px solid ${C.border}`, borderTop: `3px solid ${color}`, borderRadius: 16, padding: "26px 26px 24px", boxShadow: "0 2px 10px rgba(0,0,0,0.04)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <span style={{ fontSize: 24 }}>{icon}</span>
        <span style={{ color: C.textFaint, fontSize: 12.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1.2 }}>{label}</span>
      </div>
      <div style={{ color, fontSize: 44, fontWeight: 800, letterSpacing: -1, lineHeight: 1, fontFamily: "'Space Grotesk', sans-serif" }}>{value}</div>
      {sub && <div style={{ color: C.textFaint, fontSize: 12.5, marginTop: 8 }}>{sub}</div>}
    </div>
  );
}
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: `radial-gradient(circle at 30% 20%, #ede5d0 0%, ${C.bgDark} 55%)`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Inter', sans-serif" }}>
      <div style={{ background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 20, padding: "48px 40px", width: 380, animation: shake ? "shake 0.4s ease" : "none", boxShadow: "0 12px 40px rgba(0,0,0,0.08)" }}>
...
function MicrosoftSignIn({ onUseSampleData }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: `radial-gradient(circle at 30% 20%, #ede5d0 0%, ${C.bgDark} 55%)`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Inter', sans-serif" }}>
      <div style={{ background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 20, padding: "48px 40px", width: 420, textAlign: "center", boxShadow: "0 12px 40px rgba(0,0,0,0.08)" }}>
        <div style={{ width: 52, height: 52, background: C.gold, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 26, fontWeight: 900, color: "#1a1a1a", fontFamily: "'Space Grotesk', sans-serif" }}>E</div>
...
// Downloads the currently-visible Paid History as a CSV file — respects whatever
// phase/date range is selected on the Triggers tab at the time of download.
function downloadPaidHistoryCSV(paidList, phase, dateFrom, dateTo) {
  const escapeCsv = (val) => {
    const s = String(val ?? "");
    return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s;
  };
  const header = ["Merchant", "Type", "Description", "Amount (BWP)"];
  const rows = paidList.map(t => [t.merchant, t.type, t.message, t.amount]);
  const csv = [header, ...rows].map(r => r.map(escapeCsv).join(",")).join("\n");

  const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  const rangeLabel = dateFrom || dateTo ? `_${dateFrom || "start"}_to_${dateTo || "now"}` : "";
  a.href = url;
  a.download = `exto-pay-paid-history_phase${phase}${rangeLabel}.csv`;
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[{ k: "Min", v: q.stats.min }, { k: "Avg", v: q.stats.avg }, { k: "Max", v: q.stats.max }].map(s => (
                    <div key={s.k} style={{ flex: 1, background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 8px", textAlign: "center" }}>
                      ...
                      <div style={{ color: C.goldText, fontSize: 18, fontWeight: 700 }}>{Number.isFinite(s.v) ? (Math.round(s.v * 10) / 10) : "—"}</div>
                    </div>
                  ))}
                </div>
              )}
...
// ─── MAIN DASHBOARD ──────────────────────────────────────────────────────────
function Dashboard({ onboardingEvents, payments, dataSource, lastUpdated, onRefresh, refreshing, onLogout }) {
  const [tab, setTab] = useState("dashboard");

  const [dashPhase, setDashPhase] = useState("0");
  const [visibleKPIs, setVisibleKPIs] = useState({ merchants: true, consumers: true, qualifying: true, volume: true, cashOut: true });
  ...
  const [consumersPhase, setConsumersPhase] = useState("0");
  const [consumersDateFrom, setConsumersDateFrom] = useState("");
  const [consumersDateTo, setConsumersDateTo] = useState("");
  const [consumerConfigs, setConsumerConfigs] = useState(() => loadConsumerIncentiveConfigs());
  ...
  const [paidBaselines, setPaidBaselines] = useState(() => loadPaidBaselines());
  ...
  useEffect(() => { saveConsumerIncentiveConfigs(consumerConfigs); }, [consumerConfigs]);
  useEffect(() => { saveDashboardTargets(dashTargets); }, [dashTargets]);
  useEffect(() => { savePaidTriggers(paidTriggers); }, [paidTriggers]);
  useEffect(() => { savePaidBaselines(paidBaselines); }, [paidBaselines]);
  useEffect(() => { saveAgents(agents); }, [agents]);

  // Built once whenever the agents list changes — every onboarding/merchant
  // calculation across every tab reads from this same lookup, so editing the
  // Agents tab immediately reclassifies everything everywhere in the app.
  ...
  const consumerConfig = consumerConfigs[consumersPhase] || DEFAULT_CONSUMER_INCENTIVE_CONFIG;
  const updateConsumerConfig = (key, value) => setConsumerConfigs(prev => ({ ...prev, [consumersPhase]: { ...prev[consumersPhase], [key]: value } }));
  const updateConsumerMilestone = (idx, field, value) => {
    const newMilestones = [...consumerConfig.milestones];
    newMilestones[idx] = { ...newMilestones[idx], [field]: value };
    updateConsumerConfig("milestones", newMilestones);
  };
  const addConsumerMilestone = () => updateConsumerConfig("milestones", [...consumerConfig.milestones, { count: 0, bonus: 0 }]);
  const removeConsumerMilestone = (idx) => updateConsumerConfig("milestones", consumerConfig.milestones.filter((_, i) => i !== idx));
  ...
  const dedupedOnboarding = useMemo(() => dedupeOnboardingEvents(onboardingEvents, agentLookup), [onboardingEvents, agentLookup]);
  ...
  const merchantNames = useMemo(() => getAllMerchantNames(incOnboarding), [incOnboarding, incPayments, agentLookup]);
  ...
  const dashMerchantNames = useMemo(() => getAllMerchantNames(dashOnboarding), [dashOnboarding, dashPayments, agentLookup]);
  ...
  const triggersMerchantNames = useMemo(() => getAllMerchantNames(triggersOnboarding), [triggersOnboarding, triggersPayments, agentLookup]);
  ...
  const merchantsMerchantNames = useMemo(() => getAllMerchantNames(merchantsOnboarding), [merchantsOnboarding, merchantsPayments, agentLookup]);
  ...
  // ── Consumers tab: its own independent phase + date range ──
  const consumersOnboarding = useMemo(() => dedupedOnboarding.filter(e => inRange(e.dlt_close, consumersDateFrom, consumersDateTo)), [dedupedOnboarding, consumersDateFrom, consumersDateTo]);
  const consumersPayments = useMemo(() => payments.filter(p => inRange(p.dlt_close, consumersDateFrom, consumersDateTo)), [payments, consumersDateFrom, consumersDateTo]);
  const allConsumers = useMemo(() => getAllConsumersWithMerchant(consumersOnboarding), [consumersOnboarding]);
  const consumerResults = useMemo(() =>
    allConsumers.map(c => ({ ...c, ...calculateConsumerIncentive(c.name, consumersPayments, consumerConfig) })),
    [allConsumers, consumersPayments, consumerConfig]
  );
  const consumersFiltered = consumerResults.filter(c => c.name.toLowerCase().includes(consumerSearch.toLowerCase()));
  ...
  const internalTransfersByCoordinator = useMemo(() => {
    ...
    internalPayments.forEach(p => {
      const senderIsSpecial = isAgentName(p.sender, agentLookup);
      const receiverIsSpecial = isAgentName(p.receiver, agentLookup);
  ...
  const auditMerchantNamesForTab = useMemo(() => getAllMerchantNames(auditOnboarding), [auditOnboarding, auditPayments, agentLookup]);
  ...
  const dashStats = useMemo(() => {
    ...
    let totalVolume = 0;
    const countedTxnIds = new Set(); // guards against double-counting a merchant-to-merchant transaction
    dashMerchantNames.forEach(m => {
      const r = calculateMerchantIncentive(m, dashOnboarding, dashPayments, config);
      // A transaction between two real merchants appears in BOTH merchants'
      // qualifyingTransactions — each merchant still earns incentive on it
      // individually (that's correct and unchanged), but it must only be
      // counted ONCE in the dashboard's aggregate transaction count/volume.
      r.qualifyingTransactions.forEach(p => {
        if (countedTxnIds.has(p.txn_id)) return; // already counted via the other merchant in this same transaction
        countedTxnIds.add(p.txn_id);
        totalVolume += p.amount;
      });
    });
    const totalQualifying = countedTxnIds.size;
    // Cash-out transactions: count of Redeem-type rows where a REAL merchant
    // is the sender or receiver (excludes special-account-only Redeem rows,
    // consistent with how every other metric on this dashboard is scoped).
    const merchantNameSet = new Set(dashMerchantNames);
    const cashOutTransactions = dashPayments.filter(p =>
      p.txn_type === "Redeem" && (merchantNameSet.has(p.sender) || merchantNameSet.has(p.receiver))
    ).length;
    return { totalMerchants, uniqueConsumers, totalQualifying, totalVolume, cashOutTransactions, consumerEvents };
  ...
  const onboardTrend = useMemo(() => {
    ...
    dashOnboarding.forEach(e => {
      ...
      // Sortable key (YYYY-MM-DD) so days plot in correct chronological order
      const sortKey = `${year}-${month.padStart(2, "0")}-${day.padStart(2, "0")}`;
      // Readable label for the X-axis, e.g. "18 Jun"
      const monthNames = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
      const label = `${day} ${monthNames[parseInt(month, 10) - 1]}`;
      byDay[sortKey] = byDay[sortKey] || { label, merchants: new Set(), consumers: new Set() };
      if (e.isMerchantOnboarding) byDay[sortKey].merchants.add(e.user_name);
      else byDay[sortKey].consumers.add(e.user_name);
    });
    // Cumulative running total: each day's count is added on top of every
    // prior day's running total, so the line always trends upward over time.
    let cumMerchants = 0;
    let cumConsumers = 0;
    return Object.entries(byDay).sort(([a], [b]) => a.localeCompare(b))
      .map(([, v]) => {
        cumMerchants += v.merchants.size;
        cumConsumers += v.consumers.size;
        return { day: v.label, merchants: cumMerchants, consumers: cumConsumers };
      });
  }, [dashOnboarding]);
  ...
  const triggers = useMemo(() => {
    ...
    triggersResults.forEach(r => {
      ...
      // Onboarding and Transaction Incentive are RUNNING TOTALS, not one-time events —
      // a merchant onboarding more consumers later, or doing more transactions later,
      // must NOT silently count as "already paid" just because the trigger ID matches
      // an earlier, smaller total. Each tracks a "baseline" (the total as of the last
      // time it was marked paid); the displayed/payable amount is current total minus
      // that baseline, so only the NEW amount since last payment is ever shown as due.
      const onbId = `${r.merchantName}-onb`;
      const onbBaseline = paidBaselines[onbId] || 0;
      const onbDue = Math.round((r.onboardingIncentive - onbBaseline) * 100) / 100;
      if (onbDue > 0) {
        list.push({
          id: onbId, merchant: r.merchantName, type: "Onboarding",
          message: `${r.cappedConsumers} consumers onboarded (BWP ${r.onboardingIncentive} total, BWP ${onbBaseline} already settled)`,
          amount: onbDue, color: C.blue, isRunning: true, currentTotal: r.onboardingIncentive,
        });
      }

      const txnIncentiveTotal = Math.round((r.fixedIncentive + r.variableIncentive) * 100) / 100;
      const txnId = `${r.merchantName}-txnIncentive`;
      const txnBaseline = paidBaselines[txnId] || 0;
      const txnDue = Math.round((txnIncentiveTotal - txnBaseline) * 100) / 100;
      if (txnDue > 0) {
        list.push({
          id: txnId, merchant: r.merchantName, type: "Transaction Incentive",
          message: `${r.netCount} qualifying transactions (fixed BWP ${r.fixedIncentive} + variable BWP ${r.variableIncentive} total, BWP ${txnBaseline} already settled)`,
          amount: txnDue, color: C.blue, expandable: true, isRunning: true, currentTotal: txnIncentiveTotal,
    ...
    return list;
  }, [triggersResults, triggersConfig, paidBaselines]);

  const pendingTriggers = triggers.filter(t => !paidTriggers.has(t.id) && (triggerFilter === "all" || t.type === triggerFilter));
  // Paid history: discrete one-time triggers that were marked paid, PLUS running-total
  // triggers whose baseline is greater than 0 (meaning some amount was settled at some
  // point) — shown with the amount that was paid as of that settlement, not today's total.
  const paidList = [
    ...triggers.filter(t => !t.isRunning && paidTriggers.has(t.id)),
    ...Object.entries(paidBaselines)
      .filter(([id, amt]) => amt > 0)
      .map(([id, amt]) => {
        const parts = id.split("-");
        const type = id.endsWith("-onb") ? "Onboarding" : "Transaction Incentive";
        const merchant = id.replace(/-onb$|-txnIncentive$/, "");
        return { id, merchant, type, message: "Settled as of last payment", amount: amt, color: C.blue };
      }),
  ];
  ...
  const markPaid = (trigger) => {
    if (trigger.isRunning) {
      // Settle: update the baseline to the current total, so only future NEW
      // amounts beyond this point will ever show as due again.
      setPaidBaselines(prev => ({ ...prev, [trigger.id]: trigger.currentTotal }));
    } else {
      setPaidTriggers(prev => new Set([...prev, trigger.id]));
    }
  };

  // Captures the entire Dashboard tab's current content (KPIs, targets, chart,
  // survey insights) as a visual snapshot and assembles it into a downloadable
  // multi-page PDF — exactly as it looks on screen at the moment of download.
  const handleDownloadDashboardPDF = async () => {
    ...
    try {
      ...
      const canvas = await window.html2canvas(node, { backgroundColor: C.bgDark, scale: 2, useCORS: true });
      ...
      pdf.save(`exto-pay-dashboard_phase${dashPhase}_${dateStr}.pdf`);
    } catch (err) {
    ...
    <div style={{ minHeight: "100vh", background: C.bgDark, fontFamily: "'Inter', system-ui, sans-serif", color: C.cream }}>
      <div style={{ background: C.panelDark, borderBottom: `1px solid ${C.border}`, padding: "0 32px", display: "flex", alignItems: "center", height: 64, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginRight: 16 }}>
          ...
          <button onClick={onLogout} style={{ background: C.bgDark, border: `1px solid ${C.border}`, borderRadius: 8, padding: "6px 12px", color: C.textMuted, fontSize: 12, fontWeight: 500, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
            🔒 Logout
          </button>
        </div>
      ...
      <div style={{ padding: "24px 32px", maxWidth: 1440, margin: "0 auto" }}>
        ...
        {tab === "dashboard" && (
          <div id="dashboard-export-area">
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
              <button onClick={handleDownloadDashboardPDF} disabled={exportingPDF} style={{ background: "transparent", color: C.goldText, border: `1px solid ${C.gold}55`, borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 600, cursor: exportingPDF ? "default" : "pointer", opacity: exportingPDF ? 0.6 : 1 }}>
                {exportingPDF ? "Generating PDF…" : "⬇ Download Dashboard PDF"}
              </button>
            </div>
            <div style={{ background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 20px", marginBottom: 20, display: "flex", gap: 20, flexWrap: "wrap", alignItems: "center" }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 16, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
              <span style={{ color: C.textFaint, fontSize: 11, fontWeight: 600 }}>SHOW</span>
              {[
                { key: "merchants", label: "Merchants" },
                { key: "consumers", label: "Consumers" },
                { key: "qualifying", label: "Qualifying Txns" },
                { key: "volume", label: "Volume" },
                { key: "cashOut", label: "Cash-out Txns" },
              ].map(opt => (
                <label key={opt.key} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, color: visibleKPIs[opt.key] ? C.cream : C.textFaint, cursor: "pointer" }}>
                  <input type="checkbox" checked={visibleKPIs[opt.key]} onChange={() => setVisibleKPIs(prev => ({ ...prev, [opt.key]: !prev[opt.key] }))} style={{ accentColor: C.gold, width: 14, height: 14 }} />
                  {opt.label}
                </label>
              ))}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 18, marginBottom: 28 }}>
              {visibleKPIs.merchants && <KPICard icon="🏪" label="Merchants" value={dashStats.totalMerchants} color={C.gold} />}
              {visibleKPIs.consumers && <KPICard icon="👥" label="Consumers" value={dashStats.uniqueConsumers} color={C.green} />}
              {visibleKPIs.qualifying && <KPICard icon="⚡" label="Qualifying Txns" value={dashStats.totalQualifying} color={C.blue} />}
              {visibleKPIs.volume && <KPICard icon="💰" label="Volume" value={`BWP ${Math.round(dashStats.totalVolume).toLocaleString()}`} color={C.gold} />}
              {visibleKPIs.cashOut && <KPICard icon="🏦" label="Cash-out Txns" value={dashStats.cashOutTransactions} color={C.red} />}
              {!visibleKPIs.merchants && !visibleKPIs.consumers && !visibleKPIs.qualifying && !visibleKPIs.volume && !visibleKPIs.cashOut && (
                <div style={{ color: C.textFaint, fontSize: 13, padding: "20px 0" }}>All KPI cards are hidden — tick a box above to show one.</div>
              )}
            </div>
            ...
            <div style={{ background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20, marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <NumberInput label="Cash-out target" value={currentTarget.cashOutTransactions} onChange={v => updateTarget("cashOutTransactions", v)} />
                </div>
              </div>
              ...
              <TargetBar label="Cash-out transactions" actual={dashStats.cashOutTransactions} target={currentTarget.cashOutTransactions} />
            </div>

            <div style={{ background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20, marginBottom: 20 }}>
              <div style={{ fontWeight: 700, marginBottom: 14 }}>Onboarding Trend — Cumulative (Merchants + Consumers)</div>
              <ResponsiveContainer width="100%" height={260}>
                <LineChart data={onboardTrend} margin={{ bottom: 30 }}>
                  <XAxis dataKey="day" tick={{ fill: C.textFaint, fontSize: 11 }} angle={-35} textAnchor="end" interval={onboardTrend.length > 20 ? "preserveStartEnd" : 0} height={50} />
                  <YAxis tick={{ fill: C.textFaint, fontSize: 11 }} allowDecimals={false} />
                  <Tooltip contentStyle={{ background: C.panelDark2, border: `1px solid ${C.border}`, color: C.cream }} />
                  <Line type="monotone" dataKey="merchants" stroke={C.gold} strokeWidth={2} dot={{ r: 3 }} />
                  <Line type="monotone" dataKey="consumers" stroke={C.green} strokeWidth={2} dot={{ r: 3 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
              ...
              {filteredResults.map(r => (
                <div key={r.merchantName} style={{ background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 8, fontSize: 12 }}>
                    ...
                    <div><div style={{ color: C.textFaint }}>Variable</div><div style={{ color: C.goldText, fontWeight: 700 }}>BWP {r.variableIncentive}</div></div>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 16, marginBottom: 20, alignItems: "center", flexWrap: "wrap" }}>
              <div style={{ background: C.panelDark, border: `1px solid ${C.gold}55`, borderRadius: 12, padding: "16px 24px" }}>
                ...
                <div style={{ color: C.goldText, fontSize: 28, fontWeight: 700 }}>BWP {totalDue.toLocaleString()}</div>
              </div>
                ...
                {items.map(t => (
                  <div key={t.id} style={{ borderTop: `1px solid ${C.border}` }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0" }}>
                      ...
                      <button onClick={() => markPaid(t)} style={{ background: C.green + "22", color: C.green, border: `1px solid ${C.green}55`, borderRadius: 6, padding: "4px 10px", fontSize: 11, cursor: "pointer" }}>Mark as Paid</button>
                    </div>
                            ...
                            {t.qualifyingTransactions.map((p, i) => {
                              ...
                              return (
                                <tr key={i}>
                                  ...
                                  <td style={{ padding: "4px 8px", color: C.goldText }}>{p.amount}</td>
            ...
            {paidList.length > 0 && (
              <div style={{ marginTop: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <div style={{ fontWeight: 700, color: C.textMuted }}>📜 Paid History ({paidList.length})</div>
                  <button onClick={() => downloadPaidHistoryCSV(paidList, triggersPhase, triggersDateFrom, triggersDateTo)} style={{ background: "transparent", color: C.goldText, border: `1px solid ${C.gold}55`, borderRadius: 7, padding: "5px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                    ⬇ Download CSV
                  </button>
                </div>
                {paidList.map(t => (
        ...
        {tab === "merchants" && (
          <div>
            ...
            <div style={{ background: "#3a2a0c", border: `1px solid ${C.amber}`, borderRadius: 10, padding: 14, marginBottom: 20, fontSize: 11.5, maxHeight: 400, overflowY: "auto" }}>
              <div style={{ color: C.amber, fontWeight: 700, marginBottom: 6 }}>🔍 DEBUG — raw classification for every onboarding event (remove once resolved):</div>
              {merchantsOnboarding.map((e, i) => (
                <div key={i} style={{ color: C.textMuted, fontFamily: "monospace", marginBottom: 3, borderBottom: `1px solid ${C.border}`, paddingBottom: 3 }}>
                  name="{e.user_name}" · isMerchant={String(e.isMerchantOnboarding)} · authorize="{e.authorize}" · agent_name="{e.agent_name}" · owner="{e.owner}"
                </div>
              ))}
            </div>
                        ...
                        <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
                          ...
                          <tbody>
                            {r.qualifyingTransactions.slice(0, 15).map((p, i) => (
                              <tr key={i}><td style={{ padding: "4px 8px", color: C.textMuted }}>{p.txn_type}</td><td style={{ padding: "4px 8px" }}>{p.sender}</td><td style={{ padding: "4px 8px" }}>{p.receiver}</td><td style={{ padding: "4px 8px", color: C.goldText }}>{p.amount}</td></tr>
                            ))}
                          </tbody>
                        </table>
                      </div>
        ...
        {tab === "consumers" && (
          <div>
            <PhaseDateFilterBar phase={consumersPhase} setPhase={setConsumersPhase} dateFrom={consumersDateFrom} setDateFrom={setConsumersDateFrom} dateTo={consumersDateTo} setDateTo={setConsumersDateTo} />

            <div style={{ background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 14, padding: 20, marginBottom: 16 }}>
              <div style={{ fontWeight: 700, marginBottom: 6 }}>Phase {consumersPhase} Consumer Incentive Variables</div>
              <div style={{ color: C.textFaint, fontSize: 12, marginBottom: 14 }}>
                Consumers are not currently paid any incentive — every value below defaults to 0. Enter real numbers here whenever that changes, and the calculation below updates immediately, using the same logic as merchant incentives.
              </div>
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 16 }}>
                <NumberInput label="Activation bonus (BWP)" value={consumerConfig.activationBonus} onChange={v => updateConsumerConfig("activationBonus", v)} />
                <NumberInput label="Activation threshold" value={consumerConfig.activationThreshold} onChange={v => updateConsumerConfig("activationThreshold", v)} suffix="txns" />
                <NumberInput label="Fixed rate (BWP)" value={consumerConfig.fixedRate} onChange={v => updateConsumerConfig("fixedRate", v)} />
                <NumberInput label="Variable rate" value={consumerConfig.variableRate} onChange={v => updateConsumerConfig("variableRate", v)} suffix="%" />
                <NumberInput label="Variable cap (BWP)" value={consumerConfig.variableCap} onChange={v => updateConsumerConfig("variableCap", v)} />
                <NumberInput label="Txn cap" value={consumerConfig.txnCap} onChange={v => updateConsumerConfig("txnCap", v)} />
              </div>
              <div style={{ color: C.textFaint, fontSize: 11, fontWeight: 600, marginBottom: 8 }}>MILESTONE TIERS</div>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                {consumerConfig.milestones.map((ms, i) => (
                  <div key={i} style={{ display: "flex", gap: 6, alignItems: "flex-end", background: C.bgDark, padding: 8, borderRadius: 8, border: `1px solid ${C.border}` }}>
                    <NumberInput label="At txns" value={ms.count} onChange={v => updateConsumerMilestone(i, "count", v)} width={55} />
                    <NumberInput label="Bonus BWP" value={ms.bonus} onChange={v => updateConsumerMilestone(i, "bonus", v)} width={55} />
                    <button onClick={() => removeConsumerMilestone(i)} style={{ background: C.red + "22", color: C.red, border: "none", borderRadius: 6, padding: "5px 9px", cursor: "pointer", fontSize: 12 }}>✕</button>
                  </div>
                ))}
                <button onClick={addConsumerMilestone} style={{ background: C.green + "22", color: C.green, border: `1px solid ${C.green}55`, borderRadius: 6, padding: "8px 12px", cursor: "pointer", fontSize: 12 }}>+ Add Tier</button>
              </div>
            </div>

            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
              <div style={{ fontWeight: 700, fontSize: 16 }}>All Consumers ({allConsumers.length})</div>
              <input placeholder="🔍 Search..." value={consumerSearch} onChange={e => setConsumerSearch(e.target.value)} style={{ background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 10, padding: "8px 14px", color: C.cream, fontSize: 13, outline: "none", width: 220 }} />
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {consumersFiltered.map((c, idx) => (
                <div key={c.owner || idx} style={{ border: `1px solid ${expandedConsumer === (c.owner || idx) ? C.gold + "55" : C.border}`, borderRadius: 12, overflow: "hidden" }}>
                  <div onClick={() => setExpandedConsumer(expandedConsumer === (c.owner || idx) ? null : (c.owner || idx))} style={{ display: "flex", alignItems: "center", padding: "12px 18px", cursor: "pointer", background: expandedConsumer === (c.owner || idx) ? C.gold + "0c" : C.panelDark, gap: 14 }}>
                    <span style={{ flex: 1, fontWeight: 600 }}>{c.name}</span>
                    <span style={{ color: C.textFaint, fontSize: 12 }}>Onboarded by <b style={{ color: C.green }}>{c.onboardedBy}</b></span>
                    <span style={{ color: C.textFaint, fontSize: 12 }}>{c.netCount} qualifying txns</span>
                    <Badge color={C.gold}>BWP {c.totalIncentive.toLocaleString()}</Badge>
                    <span style={{ color: C.textFaint }}>{expandedConsumer === (c.owner || idx) ? "▲" : "▼"}</span>
                  </div>
                  {expandedConsumer === (c.owner || idx) && (
                    <div style={{ background: C.bgDark, padding: 16, borderTop: `1px solid ${C.border}` }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, fontSize: 12, marginBottom: 14 }}>
                        <div><div style={{ color: C.textFaint }}>Activation</div><div style={{ color: c.activationEligible ? C.green : C.textFaint, fontWeight: 700 }}>{c.activationEligible ? `BWP ${consumerConfig.activationBonus}` : "Pending"}</div></div>
                        <div><div style={{ color: C.textFaint }}>Fixed ({c.cappedCount} txns)</div><div style={{ color: C.blue, fontWeight: 700 }}>BWP {c.fixedIncentive}</div></div>
                        <div><div style={{ color: C.textFaint }}>Variable</div><div style={{ color: C.goldText, fontWeight: 700 }}>BWP {c.variableIncentive}</div></div>
                        <div><div style={{ color: C.textFaint }}>Milestones</div><div style={{ color: C.amber, fontWeight: 700 }}>BWP {c.milestoneIncentive}</div></div>
                      </div>
                      <div style={{ color: C.textFaint, fontSize: 11, marginBottom: 8 }}>QUALIFYING TRANSACTIONS ({c.qualifyingTransactions.length})</div>
                      <div style={{ overflowX: "auto" }}>
                        <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
                          <thead><tr>{["Type", "Sender", "Receiver", "Amount"].map(h => <th key={h} style={{ textAlign: "left", color: C.textFaint, padding: "4px 8px", borderBottom: `1px solid ${C.border}` }}>{h}</th>)}</tr></thead>
                          <tbody>
                            {c.qualifyingTransactions.slice(0, 15).map((p, i) => (
                              <tr key={i}><td style={{ padding: "4px 8px", color: C.textMuted }}>{p.txn_type}</td><td style={{ padding: "4px 8px" }}>{p.sender}</td><td style={{ padding: "4px 8px" }}>{p.receiver}</td><td style={{ padding: "4px 8px", color: C.goldText }}>{p.amount}</td></tr>
                            ))}
                            {c.qualifyingTransactions.length === 0 && <tr><td colSpan={4} style={{ padding: "8px", color: C.textFaint, textAlign: "center" }}>No qualifying transactions yet.</td></tr>}
                          </tbody>
                        </table>
                      </div>
                    </div>
              ...
              {consumersFiltered.length === 0 && <div style={{ color: C.textFaint, padding: 20, textAlign: "center" }}>No consumers found for this date range / search.</div>}
            </div>
          </div>
        )}
                        ...
                        <div key={counterparty} style={{ marginBottom: 10, border: `1px solid ${C.border}`, borderRadius: 8, overflow: "hidden" }}>
                          ...
                          {expandedCounterparty === `${coordinator}|${counterparty}` && (
                            <table style={{ width: "100%", fontSize: 12, borderCollapse: "collapse" }}>
                              ...
                              <tbody>
                                {cp.txns.map((t, i) => (
                                  <tr key={i}><td style={{ padding: "4px 10px", color: C.textMuted }}>{t.dlt_close?.split(",")[0]}</td><td style={{ padding: "4px 10px", color: C.textMuted }}>{t.txn_type}</td><td style={{ padding: "4px 10px" }}>{t.sender === coordinator ? "Sent" : "Received"}</td><td style={{ padding: "4px 10px", color: C.goldText }}>{t.amount}</td></tr>
                                ))}
                              </tbody>
                            </table>
                          )}
                ...
                <tbody>
                  {auditFiltered.map((t, i) => (
                    <tr key={i} style={{ borderBottom: `1px solid ${C.border}` }}>
                      ...
                      <td style={{ padding: "9px 12px", color: C.goldText, whiteSpace: "nowrap" }}>BWP {t.amount}</td>
        ...
        {tab === "agents" && (
          <div>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 6 }}>Agents</div>
            <div style={{ color: C.textFaint, fontSize: 12.5, marginBottom: 16 }}>
              The single source of truth for who counts as an agent. When the Authorize column in onboarding matches an agent below (by ID or by name), the person they onboard becomes a merchant. Anyone else onboarding someone makes that person a consumer instead. Marking an agent inactive only affects new onboarding going forward — anyone already onboarded by them stays classified as a merchant.
            </div>

            <button onClick={() => { setEditingAgent({ id: "", name: "", accountIds: [""], dateAdded: new Date().toISOString().slice(0, 10), active: true, notes: "", permanent: false }); setShowAgentForm(true); }} style={{ background: C.gold, color: "#1a1a1a", border: "none", borderRadius: 8, padding: "9px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer", marginBottom: 16 }}>
              + Add Agent
            </button>

            {showAgentForm && editingAgent && (
              <div style={{ background: C.panelDark, border: `1px solid ${C.gold}55`, borderRadius: 12, padding: 18, marginBottom: 18 }}>
                <div style={{ fontWeight: 700, marginBottom: 12 }}>{agents.some(a => a.id === editingAgent.id) ? "Edit Agent" : "New Agent"}</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 12 }}>
                  <div>
                    <label style={{ color: C.textFaint, fontSize: 11, display: "block", marginBottom: 4 }}>Name</label>
                    <input value={editingAgent.name} onChange={e => setEditingAgent({ ...editingAgent, name: e.target.value })} placeholder="e.g. Thabo Khumalo" style={{ width: "100%", maxWidth: 340, padding: "8px 12px", background: C.bgDark, border: `1px solid ${C.border}`, borderRadius: 7, color: C.cream, fontSize: 13, boxSizing: "border-box" }} />
                  </div>
                  <div>
                    <label style={{ color: C.textFaint, fontSize: 11, display: "block", marginBottom: 4 }}>Account ID(s) — one per line (a person may have multiple from card + phone wallet activations)</label>
                    <textarea value={editingAgent.accountIds.join("\n")} onChange={e => setEditingAgent({ ...editingAgent, accountIds: e.target.value.split("\n") })} rows={3} placeholder="rAbc123..." style={{ width: "100%", maxWidth: 340, padding: "8px 12px", background: C.bgDark, border: `1px solid ${C.border}`, borderRadius: 7, color: C.cream, fontSize: 12, fontFamily: "monospace", boxSizing: "border-box" }} />
                  </div>
                  <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                    <div>
                      <label style={{ color: C.textFaint, fontSize: 11, display: "block", marginBottom: 4 }}>Date Added</label>
                      <input type="date" value={editingAgent.dateAdded} onChange={e => setEditingAgent({ ...editingAgent, dateAdded: e.target.value })} style={{ padding: "8px 12px", background: C.bgDark, border: `1px solid ${C.border}`, borderRadius: 7, color: C.cream, fontSize: 13 }} />
                    </div>
                    <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: C.cream, cursor: "pointer", marginTop: 18 }}>
                      <input type="checkbox" checked={editingAgent.active} onChange={e => setEditingAgent({ ...editingAgent, active: e.target.checked })} style={{ accentColor: C.gold, width: 15, height: 15 }} />
                      Active
                    </label>
                  </div>
                  <div>
                    <label style={{ color: C.textFaint, fontSize: 11, display: "block", marginBottom: 4 }}>Notes</label>
                    <textarea value={editingAgent.notes} onChange={e => setEditingAgent({ ...editingAgent, notes: e.target.value })} rows={2} placeholder="Optional notes" style={{ width: "100%", maxWidth: 340, padding: "8px 12px", background: C.bgDark, border: `1px solid ${C.border}`, borderRadius: 7, color: C.cream, fontSize: 13, boxSizing: "border-box" }} />
                  </div>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  <button
                    onClick={() => {
                      const cleanedIds = editingAgent.accountIds.map(id => id.trim()).filter(Boolean);
                      if (!editingAgent.name.trim() || cleanedIds.length === 0) {
                        alert("Name and at least one Account ID are required.");
                        return;
                      }
                      const isNew = !agents.some(a => a.id === editingAgent.id);
                      const finalAgent = {
                        ...editingAgent,
                        accountIds: cleanedIds,
                        id: editingAgent.id || editingAgent.name.toLowerCase().replace(/[^a-z0-9]+/g, "-") + "-" + Date.now(),
                      };
                      setAgents(prev => isNew ? [...prev, finalAgent] : prev.map(a => a.id === finalAgent.id ? finalAgent : a));
                      setShowAgentForm(false);
                      setEditingAgent(null);
                    }}
                    style={{ background: C.green + "22", color: C.green, border: `1px solid ${C.green}55`, borderRadius: 7, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
                  >
                    Save
                  </button>
                  <button onClick={() => { setShowAgentForm(false); setEditingAgent(null); }} style={{ background: "transparent", color: C.textMuted, border: `1px solid ${C.border}`, borderRadius: 7, padding: "8px 16px", fontSize: 13, cursor: "pointer" }}>
                    Cancel
                  </button>
                </div>
              </div>
            ...
            <div style={{ overflowX: "auto", background: C.panelDark, border: `1px solid ${C.border}`, borderRadius: 12 }}>
              <table style={{ width: "100%", fontSize: 12.5, borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: C.panelDark2 }}>
                    {["Name", "Account ID(s)", "Date Added", "Active", "Notes", ""].map(h => (
                      <th key={h} style={{ textAlign: "left", color: C.textFaint, padding: "10px 12px", borderBottom: `1px solid ${C.border}`, fontWeight: 600, whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {agents.map(a => (
                    <tr key={a.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                      <td style={{ padding: "9px 12px", color: C.cream, fontWeight: 600 }}>
                        {a.name}{a.permanent && <Badge color={C.amber}>Permanent</Badge>}
                      </td>
                      <td style={{ padding: "9px 12px", color: C.textFaint, fontFamily: "monospace", fontSize: 11 }}>
                        {a.accountIds.map((id, i) => <div key={i}>{id}</div>)}
                      </td>
                      <td style={{ padding: "9px 12px", color: C.textMuted, whiteSpace: "nowrap" }}>{a.dateAdded}</td>
                      <td style={{ padding: "9px 12px" }}>
                        <button
                          onClick={() => setAgents(prev => prev.map(x => x.id === a.id ? { ...x, active: !x.active } : x))}
                          style={{ background: a.active ? C.green + "22" : C.textFaint + "22", color: a.active ? C.green : C.textFaint, border: `1px solid ${a.active ? C.green : C.textFaint}55`, borderRadius: 6, padding: "3px 10px", fontSize: 11, cursor: "pointer", fontWeight: 600 }}
                        >
                          {a.active ? "Active" : "Inactive"}
                        </button>
                      </td>
                      <td style={{ padding: "9px 12px", color: C.textFaint, maxWidth: 200 }}>{a.notes}</td>
                      <td style={{ padding: "9px 12px", whiteSpace: "nowrap" }}>
                        <button onClick={() => { setEditingAgent({ ...a }); setShowAgentForm(true); }} style={{ background: "transparent", color: C.blue, border: `1px solid ${C.blue}55`, borderRadius: 6, padding: "3px 10px", fontSize: 11, cursor: "pointer", marginRight: 6 }}>Edit</button>
                        {!a.permanent && (
                          <button onClick={() => { if (window.confirm(`Remove ${a.name} from the agents list? Past onboarding they performed stays classified as merchant activity.`)) setAgents(prev => prev.filter(x => x.id !== a.id)); }} style={{ background: "transparent", color: C.red, border: `1px solid ${C.red}55`, borderRadius: 6, padding: "3px 10px", fontSize: 11, cursor: "pointer" }}>Remove</button>
                        )}
                      </td>
                    </tr>
                  ))}
...
function LiveDataApp({ onLogout }) {
  ...
  return (
    <Dashboard onboardingEvents={onboardingEvents} payments={payments} dataSource={dataSource} lastUpdated={lastUpdated} onRefresh={loadLiveData} refreshing={refreshing} onLogout={onLogout} />
  );
}
...
export default function App() {
  ...
  return (
    <MsalProvider instance={msalInstance}>
      <LiveDataApp onLogout={() => setLoggedIn(false)} />
    </MsalProvider>
  );
}
